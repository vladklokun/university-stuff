		...
		\subsection{Коло \schel{RL}}
			...
			\centering
				\begin{tikzpicture}[scale=0.5]
					%%% Plot axes and starting point
					\coordinate [label = {below left:$0$}] (O) at (0,0) ;
					\coordinate [label = {below:$+1$}]     (X) at (11,0) ;
					\coordinate [label = {left:$+j$}]      (Y) at (0,8) ;
					\draw[<->, thick] (X) -- (O) -- (Y);
					%%%
					
					%%% Diagram points
					% Total voltage
					\coordinate (UR)  at (7.7,0);
					\coordinate (U)   at (xyz polar cs: angle = 33, radius = 10.9);
					\coordinate (URK) at ($(O)!(U)!(X)$);
					%%%
					
					%%%
					\draw[->] (O)   -- node [below] {$U_R$} (UR);
					\draw[->] (0,0) -- node [above] {$U$} (U);
					\pic ["$\varphi$", draw, angle eccentricity = 1.5] {angle = X--O--U};
					\draw[->] (UR)  -- node [left] {$U_K$} (U);
					\draw[<-] (U)   -- node [right] {$U_{X_{K}}$} (URK);
					\draw[->] (UR)  -- node [below] {$U_{R_K}$} (URK);
					
					\pic ["$\varphi_x$", draw, angle radius = 0.25cm, angle eccentricity = 2.25] {angle = X--UR--U};
					%%%
				\end{tikzpicture}
			...
			Оскільки $\angle \varphi > 0$, трикутники будуть називатись активно-індуктивними.
		
		\subsection{Коло \schel{RC}}
			Оскільки $\angle \varphi < 0$, трикутники будуть називатись активно-ємнісними.
