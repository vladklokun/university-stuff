...
pokaznyk(Number, PEC) :-
% Find motorcycles with engine capacity EC == PEC
% PEC - pokaznyk engine_capacity za yakim sortuutsa zapisi
	moto(Number, _, _, _, EC,_, _, _), PEC = EC. 
	

plc(C,S) :-
% Показує, у скілької мотоциклів значення об'єму двигуна менше за задане 
	zapysiv(Z), plcc(Z,C,S).

plcc(0, _, S) :-
% Рахує, скільки мотоциклів
	S=0, !.  

plcc(N, C, S) :-
	N1=N-1, plcc(N1, C, S1), pokaznyk(N, PEC), PEC < C, S = S1+1.   
...
print_s(Zhel_Mesto) :-
	...
	Zhel_Mesto>Z, !.
print_s(Zhel_Mesto) :-
	print_ss(1, Zhel_Mesto).  

print_ss(Nstroki, _) :- zapysiv(Z), Nstroki>Z, !.	
								

print_ss(Nstroki, Zhel_Mesto) :-
	pokaznyk(Nstroki, PEC),
	plc(PEC, S),
	S = Zhel_Mesto-1,
	moto(Nstroki, Name, _, _, _, _, _,_),
	write(Name, " ", PEC ), nl,
...
print_ss(Nstroki, Zhel_Mesto) :-
	...
	S \= Zhel_Mesto-1,
	N1 = Nstroki+1,
	print_ss(N1, Zhel_Mesto). %if ne znaideno mensche znachenna, to perehid do obrobki nastupnogo znachenna  
...
kolvon(L,K_N) :-
	N1=L-1,
	kolvon(N1,K1_N1),
	moto(L,_,_,_,_,_,G,_),
	customer(_,sano,_,_,_,_,G1),
	G @=< G1,
	K_N=K1_N1+1.
